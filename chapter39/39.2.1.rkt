...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname 39.2.1) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require htdp/draw)
...
(define (remove-light x-posn)
  (and (clear-solid-disk (make-posn x-posn Y-RED) BULB-RADIUS)
       (clear-solid-disk (make-posn x-posn Y-YELLOW) BULB-RADIUS)
       (clear-solid-disk (make-posn x-posn Y-GREEN) BULB-RADIUS)))
  ...
  (local (;; current-color : TL-color
          ...
          (define (service-manager msg)
            (cond
              ...
              [(symbol=? msg 'delete) (delete)]
              [(symbol=? msg 'name) (street-name)]
...
(define (make-city city-name)
  (local ((define city-name empty)
          (define (name) city-name)
          (define (add street-name x-pos)
            (set! city-name
                  (cons (make-traffic-light street-name x-pos)
                        city-name)))
          (define (remove street-name)
            (set! city-name
                  (filter (lambda (a-light)
                            (cond
                              [(equal? street-name (a-light 'name))
                               (begin
                                 (a-light 'delete)
                                 false)]
                              [else true]))
                          city-name)))
          (define (switch street-name)
            (map (lambda (a-light)
                   (cond
                     [(equal? street-name (a-light 'name))
                      (a-light 'next)]
          ...
          (define (service-manager msg)
            (cond
              ...
              [(symbol=? msg 'switch) switch]
              [(symbol=? msg 'reset) reset]
              [else (error 'make-city "message not understood")])))
    service-manager))


(define London (make-city 'London))
((London 'add) 'sunrise@rice 50)
((London 'add) 'sunrise@cmu 150)