...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname 15.3.4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(define (find a-wp a-word)
  (cond
    [(not (occurs a-wp a-word)) false]
    [else
     (append (list (wp-header a-wp)) (find-path (wp-body a-wp) a-word))]))

(define (find-path lod a-word)
  (cond
    [(empty? lod) '()]
    [(symbol? (first lod)) (if (eq? (first lod) a-word)
                               '()
                               (find-path (rest lod) a-word))]
    [else (append (find (first lod) a-word)
                  (find-path (rest lod) a-word))]))
